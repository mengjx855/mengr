...
scRNA_marker_db_build <- function(
  ...
  key_vec <- paste(
    data_df$species, data_df$tissue_class, data_df$cell_name, sep = '|'
  )
...
scRNA_feature_plots <- function(seurat, markers, reduction = 'umap') {
  ...
  row_plot_list <- purrr::imap(marker_list, \(gene_vec, group_name) {
    title_plot <- ggpubr::ggtext(
      data.frame(x = 1, y = 1), 'x', 'y', size = 14, face = 'bold',
...
cellchat_run <- function(
  ...
  if (!'data' %in% layer_vec) {
    seurat <- Seurat::NormalizeData(
      seurat, assay = assay, normalization.method = 'LogNormalize',
      scale.factor = 10000, verbose = FALSE
    )
  }

  expression_mat <- Seurat::GetAssayData(
    seurat, assay = assay, layer = 'data'
  )
  ...
  cellchat <- CellChat::computeCommunProb(
    cellchat, type = prob_method, trim = trim,
...
scRNA_summarise_genes <- function(
  ...
  count_mat <- Seurat::GetAssayData(
    seurat, assay = assay, layer = count_layer
  )[gene_vec, , drop = FALSE]
  expression_mat <- Seurat::GetAssayData(
    seurat, assay = assay, layer = expression_layer
  )[gene_vec, , drop = FALSE]
...
scRNA_expression_dotplot <- function(
  ...
  ggplot2::ggplot(
    ...
    ggplot2::facet_wrap(
      stats::as.formula(paste('~', gene_col)), nrow = facet_nrow
    ) +
