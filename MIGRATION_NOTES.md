...
- 包名由 `mengR` 改为 `mengr`，旧代码中的 `library(mengR)` 需要改为
  `library(mengr)`。
- 本地配置 option 由 `mengR.database` 改为 `mengr.database`。
- GitHub 仓库地址改为 `mengjx855/mengr`。

- 样本元数据参数 `group` 改为 `sample_meta`。
- 双数据集接口的 `group_x`、`group_y` 改为 `sample_meta_x`、`sample_meta_y`。
- feature 注释表参数 `metadata` 改为 `feature_meta`；样本层面的 `metadata` 或
  `sample_metadata` 改为 `sample_meta`。
- `sample_col`、`group_col` 及实际数据中的分组列名不变。

## 0.0.2

0.0.2 统一了全小写 snake_case 函数 API、脚本头部与逐函数帮助文档。所有函数体
或行为变化均单独记录在 [NEWS.md](NEWS.md)，内部变量命名规则见
[STYLE_GUIDE.md](STYLE_GUIDE.md)。本文件以下内容保留 0.0.1 初次迁移时的历史记录。

0.0.1 记录中出现的 `scRNA_*` 名称在 0.0.2 进一步统一为 `scrna_*`；同理，
`calcu_PCoA()`、`plot_PCoA()`、`plot_NMDS()` 和 `mengR_config()` 当前分别使用
`calcu_pcoa()`、`plot_pcoa()`、`plot_nmds()` 和 `mengr_config()`。
