...
- `sample_meta` 通常包含 `sample_col` 和 `group_col`，并按样本名与 profile 对齐。
- `feature_meta` 表示与 profile 行名匹配的 feature 注释表。
...
| 函数 | 功能描述 |
| --- | --- |
| `transform_log2()` | 对一个数值向量进行 log2 转换，并处理零值和缺失值。 |
| `transform_log10()` | 对一个数值向量进行 log10 转换，并处理零值和缺失值。 |
| `transform_clr()` | 对组成型数值向量执行 centered log-ratio 转换。 |
...
| `profile_trans_clr()` | 对每个样本执行 transform_clr 转换，保持 feature × sample 方向。 |
...
| `plot_circlepack()` | 按类别数值绘制圆形打包组成图，并保留整理后的作图数据。 |
...
| `.align_profile_group()` | 按样本名对齐 profile 与 sample metadata。 |
...
| `.prepare_rf_data()` | 为 random forest 对齐 profile、样本元数据和 outcome。 |
| `.rf_probability_df()` | 从 random-forest 模型提取类别概率。 |
| `.prepare_svm_data()` | 为 SVM 对齐 profile、样本元数据和 outcome。 |
...
- 大部分绘图依赖 `ggplot2`，组合图可能使用 `patchwork` 或 `cowplot`；
  `plot_circlepack()` 需要 `ggraph`、`tidygraph` 和 `withr`。
- 本文描述 0.0.3 源码接口；源码包已经完成构建和 `R CMD check`。是否已安装到
  某个 R library 取决于当前使用环境。
