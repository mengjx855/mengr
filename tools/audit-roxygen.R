...
rd_with_chinese <- rd_files[vapply(rd_files, function(rd_file) {
  contains_han(readLines(rd_file, warn = FALSE, encoding = "UTF-8"))
}, logical(1))]
if (length(rd_with_chinese)) {
  stop(
    "Rd files containing Chinese text: ",
    paste(basename(rd_with_chinese), collapse = ", ")
  )
}
...
cat("Rd files containing Chinese text:", length(rd_with_chinese), "\n")
